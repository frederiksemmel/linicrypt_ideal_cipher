...
The queries in the execution of $\H(\vm)$ are the same as those for $\H(\pi\vm)$,
...
Let $f$ be one of the 5 compression functions for which we found collisions only for different input lengths.
...
A general characterization of collision resistance would have to consider these two examples.

\pagebreak

Must do:
\begin{enumerate}
\item revise chapter 5 + grammerly
\end{enumerate}
