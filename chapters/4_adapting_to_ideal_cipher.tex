...
\begin{proof}
    ...
    If we assume $N \geq 2$, the probability for this is bounded by
    \[
        \frac{1}{|\F| - N} \leq \frac{(N+1)N - 2N}{2|\F| - 2N} \leq \frac{(N+1)N}{2|\F|}.
    \]
    This would contradict with the assumption in the Lemma.

    Or: Otherwise, we can force $\A$ to make these queries by modifying it,
    ...
    \leq \frac{(N+1)N}{2|\F|}.
    \end{equation*}
    The last inequality follows from the fact that $\frac{a-c}{b-c} \leq \frac{a}{b} \iff a \leq b$ for $a,b,c \in \RR_+$ with $a,b \geq c$.
    In our case we have $(N+1)N \leq 2|\F|$ for $N < \sqrt{2|\F|}$ and $(N+1)N \geq 2N$.
    If $N \geq \sqrt{2|\F|}$ the inequality holds trivially.
...
\label{corollary_single_query_cr_resistance}
    ...
    \item There is an adversary making $N$ queries that finds collisions with probability $ \geq (N+1)N / 2|\F|$.
    \end{enumerate}
\end{corollary}
